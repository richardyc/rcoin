...
{
    boost::filesystem::path pathConfigFile(GetArg("-conf", "rcoin.conf"));
                    ...
                    map<string, vector<string> >& mapMultiSettingsRet)
{
    ...
    if (!streamConfig.good())
        return; // No rcoin.conf file is OK
    ...
    {
        // Don't overwrite existing settings so command line settings override rcoin.conf
...
{
    boost::filesystem::path pathPidFile(GetArg("-pid", "rcoind.pid"));
    ...
    if (vTimeOffsets.size() >= 5 && vTimeOffsets.size() % 2 == 1)
    {
        ...
        else
        {
            ...
            if (!fDone)
            {
                ...
                {
                    fDone = true;
                    string strMessage = _("Warning: Please check that your computer's date and time are correct.  If your clock is wrong rcoin will not work properly.");
                    strMiscWarning = strMessage;
                    printf("*** %s\n", strMessage.c_str());
                    ThreadSafeMessageBox(strMessage+" ", string("rcoin"), wxOK | wxICON_EXCLAMATION);
                }
            }
        }
        if (fDebug) {
...
std::string FormatSubVersion(const std::string& name, int nClientVersion, const std::vector<std::string>& comments)
{
    ...
    ss << "/";
    ss << "rcoin:" << FormatVersion(PEERUNITY_VERSION);
    ss << "(" << CLIENT_BUILD << ")/";
    return ss.str();
}
