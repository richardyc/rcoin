...
// rcoin: get network Gh/s estimate
...
Value getwork(const Array& params, bool fHelp)
{
    ...
    if (vNodes.empty())
        throw JSONRPCError(-9, "rcoin is not connected!");

    if (IsInitialBlockDownload())
        throw JSONRPCError(-10, "rcoin is downloading blocks...");

    if (pindexBest != NULL && pindexBest->nTime > POW_END_TIME)
        throw JSONRPCError(-10, "rcoin is currently on pure PoS state");
...
string HTTPPost(const string& strMsg, const map<string,string>& mapRequestHeaders)
{
    ostringstream s;
    s << "POST / HTTP/1.1\r\n"
      << "User-Agent: rcoin-json-rpc/" << FormatFullVersion() << "\r\n"
      << "Host: 127.0.0.1\r\n"
...
void ThreadRPCServer2(void* parg)
{
        ...
        (mapArgs["-rpcuser"] == mapArgs["-rpcpassword"]))
    {
        ...
        string strWhatAmI = "To use rcoin(rcoind)";
